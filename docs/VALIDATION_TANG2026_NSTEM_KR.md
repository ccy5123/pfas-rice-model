...
> **부분 종착(terminal)**이 됩니다. **2단계 결과**(dw 보정 후, 아래 ⚠️): ① 구조가 구획 **패턴**을 치유
> (**shape RMSE 0.85 → 0.39**, 잎 부담 81%→30%); ② **f_xy 보정이 절대 레벨을 치유**(**overall RMSE 1.53 → 1.20 → 0.71**)
> — 단 **이삭/낟알은 구조적으로 과소**(fw/dw 정정으로 드러남).
...
> **GenX provisional f_xy=0.233이 18× 과대**(단쇄 PFCA 인공물)인 것. PFOS→W2(0.14), GenX→Tang보정(0.013)으로 **overall RMSE 0.71**.

> ⚠️ **정정 (단위, fresh/dry; 이 세션)** — 이 검증의 TF는 원래 **모델 *신선중* TF를 Tang *건중* TF와 무변환 비교**했습니다
> (`baseline_tf`/`nstem_tf`). TF는 노출기준(`Cw`)엔 무관하나 조직 수분기준엔 무관하지 **않아**
> `TF_dw = TF_fw·(1−θ_root)/(1−θ_tissue)`(이삭 ×0.116)를 적용해야 합니다. **수정 후 수치**(위 본문은 이미 반영):
> shape RMSE 0.84→0.11 ⇒ **0.85→0.39**, overall 1.28→1.01→0.18 ⇒ **1.53→1.20→0.71**. **질적 결론은 유지**
> (구조가 패턴을 개선·잎 부담 81%→30%·줄기 1%→29%, 레벨 레버는 f_xy), 그러나 **이삭은 모든 구조·변형·L_Ph에서 ~3–8× 과소**
> = 구조적(곡립 특이 기작 필요). 전말: **`docs/tang2026_grain_units_exploration.md`**. 그림/표의 옛 fw 수치는 이 정정으로 대체됩니다.
...
잎 독점(81%)이 줄기·잎으로 재배분. (E) RMSE 진행 — **구조가 패턴(shape 0.85→0.39), f_xy 보정이 레벨(overall 1.53→1.20→0.71)**(dw 보정).
...
**(dry-weight 기준; `TF_dw = TF_fw·(1−θ_root)/(1−θ_tissue)` 적용 — §정정 참조)**

| PFAS | 조직 | **Tang** | single-straw | nstem(mono) | **+f_xy 보정** |
|---|---|---:|---:|---:|---:|
| PFOA | stalk | 1.45 | 0.02 | 0.75 | **0.75** |
| PFOA | leaf  | 1.66 | 2.70 | 0.93 | **0.93** |
| PFOA | endosperm | 0.95 | 0.05 | 0.11 | **0.11** |
| PFOS | stalk | 0.58 | 0.00 | 0.02 | **0.21** |
| PFOS | leaf  | 0.68 | 0.07 | 0.03 | **0.26** |
| PFOS | endosperm | 0.77 | 0.00 | 0.00 | **0.03** |
| GenX | stalk | 1.10 | 0.15 | 11.9 | **0.76** |
| GenX | leaf  | 1.38 | 32.2 | 11.3 | **0.70** |
| GenX | endosperm | 1.39 | 1.69 | 2.04 | **0.13** |

**log10 RMSE 진행 (3 화합물 × 3 조직 = 9점):**

| 지표 | single-straw | nstem(mono) | **+f_xy 보정** |
|---|---:|---:|---:|
| 전체(9점) | 1.53 | 1.20 | **0.71** |
| **shape**(조직 **패턴**, 레벨 제거) | 0.85 | **0.39** | 0.39 |

→ **구조가 패턴을(0.85→0.39), f_xy 보정이 레벨을(1.20→0.71) 치유**(질적으로 유효). PFOA 식물-부담 분포: 잎 **81%→30%**, 줄기 **1%→29%**.
**단 이삭/낟알(endosperm)은 보정 후에도 3–8× 과소** — L_Ph·lipid로도 못 닫히는 **구조적** 갭(`docs/tang2026_grain_units_exploration.md`); 전체 RMSE 하한(0.71)을 이삭이 지배.
...
→ **overall RMSE 1.20 → 0.71** (dw; F 패널). PFOS는 W2(독립값)이라 Tang stalk/leaf을 약간 과소(0.21/0.26 vs 0.58/0.68)하나 방향·order 일치; **이삭은 전 화합물 구조적 과소**(0.71의 하한을 지배).

## 5. 정직한 결론
- **2단계로 Tang 재검증(dw 기준)**: ① 재배분이 지상부 **stalk·leaf 패턴**을 개선(shape RMSE 0.85→0.39);
  ② **f_xy 보정이 절대 레벨**을 치유(overall RMSE **1.53→1.20→0.71**), stalk/leaf는 Tang과 order-of-magnitude 이내.
  **단 이삭/낟알은 ~3–8× 과소(구조적; `tang2026_grain_units_exploration.md`)** — 이게 RMSE를 0.71에 묶음.
