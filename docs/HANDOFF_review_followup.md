...
## 작업 우선순위 한눈에  — **P1·P2·P3 전부 완료**
- **P1 (공개 전 권장, 윤리적 핵심)**: 일반인 모드 절대 수치에 **불확실성/등급 가시화**. ✅ **DONE**
- **P2 (사용성 다듬기)**: 역추정 지연 ✅, 헤드라인↔지도 불일치 ✅, 일반인 모드 용어 누수 ✅, 낟알 BAF<1 문구 ✅, PNG 내보내기 폴백 ✅. → **P2 전부 완료**
- **P3 (유지보수)**: `app.py` 모듈화 ✅, i18n 정리 ✅, 문서 정본 표시 ✅, lint 정리 ✅. → **P3 전부 완료**
- **별첨**: 기존 과학 백로그(이번 리뷰 범위 밖이나 분실 방지용 포인터) — **남음(데이터/실험 의존)**.

> **P3-1/P3-2 완료(이 세션)**: `app.py`(1270줄) → **`ui/` 패키지**로 분할(`__init__`/`common`/`sidebar`/`simple`/`expert`),
> `app.py`는 41줄 조립자. 상태는 단일 `SimpleNamespace cfg`로 흐르고 동작/문자열 불변. 이어서 **i18n**: 흩어진
> `if ko else` 삼항과 짝 상수(disclaimer/congener/preset/glossary)를 **`ui/i18n.py`**(`STRINGS` + `t(key,lang,**fmt)`)로
> 중앙화(공개 이름 유지 → sidebar/simple/expert 불변). 단일언어 페이지 카피는 의도적으로 simple(ko)/expert(en) 모듈에
> 잔류, `plots`의 `lang=`는 유지. 검증: compile/pyflakes clean, Playwright로 양쪽 모드·역추정·커스텀표 동치 확인.

> **진행 상황(이 세션)**: **P1 + P2-2/P2-3/P2-4 완료** — 일반인 헤드라인 메트릭 카드·요약·📈/📊 탭에
> a-priori 예측오차(log10 RMSE≈0.85 ⇒ ×/÷ ~7배) 기반 **불확실성 밴드/캡션** 부착(`model_api.predictive_band`
> /`uncertainty_factor`, 순수 함수 + 테스트), 📊 막대에 오차선(`plots.fig_where_plain(band=True)`), 지도 캡션에
> 헤드라인↔지도 일관화 문구(짚=줄기+잎 평균·부위별 농도), `plots.fig_baf(lang=)`로 일반인 BAF 축 한국어화,
> 낟알 BAF<1 → "토양수보다 낮음(약 1/N 수준)" 분기.
>
> **이어서 P2-1·P2-5·P3-3·P3-4 완료** — 역추정에 **단계별 진행바**(`estimate_exposure_bayesian(progress=…)` 콜백
> + 앱의 `st.progress` + session_state 결과 캐시; `st.cache_data` 헬퍼 제거) → "무반응처럼 보임" 해소; PNG 내보내기에
> **대화형 HTML 폴백**(`_html_bytes`, kaleido/Chrome 불요)+친절 설치 안내(`pip install kaleido && plotly_get_chrome`);
> README/README_KR 상단에 **정본 vs. 이력** 1줄; pyflakes nit 정리(app f-string, calibration/oryza 미사용 `field`/`binding_factors`,
> `pfas_rice_plant_module` `__all__`로 재export 명시 — model_api rdkit 프로브만 의도적 `# noqa` 잔존). `parameters.json`·
> 모델 수식·`reproduce_demo`(0.029) 불변. **남은 것**: P3-1(app.py 모듈화)·P3-2(i18n) — 큰 리팩터, 후속 세션(사용자 결정).
...
## P1 — 일반인 모드 수치에 불확실성 가시화 (가장 중요)  ✅ DONE

> **구현(이 세션)**: 권장안 1(b)+1 채택 — 문서화된 a-priori 예측오차(log10 RMSE≈0.85)를 곱 밴드(×/÷ ~7배)로
> 환산하는 순수 헬퍼 `model_api.predictive_band(value)`/`uncertainty_factor()`(상수 `APRIORI_LOG10_RMSE=0.85`,
> 테스트 `test_predictive_band_and_uncertainty_factor`)를 추가하고, 일반인 헤드라인의 **세 메트릭 카드**에 범위
> 델타(`대략 lo–hi µg/kg`)+카드 하단 캡션(`실측과 약 7배까지 차이날 수 있음`), 요약 문장에 범위+정성 경고, **📈/📊
> 탭 캡션**에 동일 신호, **📊 막대**에 오차선(`plots.fig_where_plain(..., band=True)`)을 부착. 결과적으로 일반인
> 기본 화면의 **모든 절대 수치 옆에 불확실성 신호**가 보임(수용 기준 충족). 전문가 모드·`parameters.json`·수식 불변.
...
### P2-1. 🔎 거꾸로 추정 지연 (8–22초)  ✅ DONE (진행바 방식)
...
### P3-3. 문서 정본 표시  ✅ DONE (README/README_KR 상단 1줄)
...
### P3-4. lint 정리  ✅ DONE (pyflakes nit; CI lint 미추가 — 범위 보수)
