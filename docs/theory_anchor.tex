...
terms is unlikely to be a property of entry at all, and the natural reading is that
Eq.~\eqref{eq:factor} is mis-partitioned --- what the fits are absorbing into $\eta$ belongs in
$\ffree$ (i.e.\ in $\Bk$/sequestration), which is where the two--pool root work
(\texttt{docs/twopool\_root\_exploration.md}) independently put it. Re-deriving the split is open
work and is gated on the same wet--lab assay as the $k_{\mathrm{seq}}$ promotion decision, so no
replacement factorisation is asserted here.
