...
\author{Consolidated from seven \texttt{sci-adk} runs\\
\small (Scientific Agentic Discovery Kit, \url{https://github.com/ccy5123/sci-adk})}
\date{\today}
...
(\textit{Oryza sativa}). It supersedes the seven machine-rendered
per-run skeleton drafts (\texttt{runs/*/paper/draft.tex}), whose
non-ASCII section bodies did not render. Every number, verdict and
record digest reported here was re-derived by \texttt{sci-adk verify}
(LLM-free replay from the frozen records); all seven runs verify with
exit~0 and every recorded claim is \textsc{reproduced}.
