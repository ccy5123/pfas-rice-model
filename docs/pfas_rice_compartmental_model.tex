...
\begin{description}[leftmargin=2.2em,style=nextline,nosep]
    ...
    \item[(A2)] Each compartment is well mixed; the concentration carried by the sap leaving a compartment is the free aqueous concentration $\Cw{}_{,k}=C_k/B_k$.
...
\begin{equation}\label{eq:binding}
    ...
    B_k=\theta_k+\rho_k\!\left(f_\mathrm{prot}K_\mathrm{prot}+f_\mathrm{PL}K_\mathrm{PL}+f_\mathrm{cw}K_\mathrm{cw}\right),
\end{equation}
with $\theta_k$ the aqueous fraction, $\rho_k$ density, and $f_\mathrm{prot},f_\mathrm{PL},f_\mathrm{cw}$ the protein, phospholipid, and cell-wall mass fractions.  Protein binding saturates at high concentration; $K_\mathrm{prot}$ may be replaced by a Langmuir form where needed.  $B_k$ carries the binding-driven accumulation; the large protein content of grain makes $B_4$ large, trapping PFAS in the edible tissue.
...
\end{equation}
and each compartment exports sap at its free concentration $C_k/B_k$.  The phloem flow is driven by grain sink demand plus a recirculating fraction $\varphi$ of transpiration:
...
\begin{align}
    \dv{C_1}{t}&=j_R-\frac{\dot Q_\mathrm{TP}}{M_1}\frac{C_1}{B_1}+\varphi\frac{\dot Q_\mathrm{Phl}}{M_1}C_\mathrm{Phl}-\gamma_1C_1-\mu_1C_1, \label{eq:root}\\[4pt]
    \dv{C_2}{t}&=\frac{\dot Q_\mathrm{TP}}{M_2}\!\left(\frac{C_1}{B_1}-\frac{C_2}{B_2}\right)-\gamma_2C_2-\mu_2C_2, \label{eq:stem}\\[4pt]
    \dv{C_3}{t}&=\frac{A_3}{A_3+A_4}\frac{\dot Q_\mathrm{TP}}{M_3}\frac{C_2}{B_2}-\frac{\dot Q_\mathrm{Phl}}{M_3}C_\mathrm{Phl}-\gamma_3C_3-\mu_3C_3, \label{eq:leaf}\\[4pt]
...
\end{align}
With $j_R=a_RJ_R$ from Eq.~\eqref{eq:JR_pfas} and the closures \eqref{eq:binding}--\eqref{eq:Cphl}, Eqs.~\eqref{eq:root}--\eqref{eq:fruit} form a closed nonlinear ODE system in $(C_1,\dots,C_4)$.
...
\begin{equation}
    \frac{u_2}{u_1}=\frac{1}{1+\mathrm{Da}_2}\;\xrightarrow{\ \gamma_2\to0\ }\;1,
\end{equation}
so for non-metabolised PFAS the free concentration is conserved along the xylem ($\Cw{}_{,2}\approx\Cw{}_{,1}$).
...
\begin{equation}\label{eq:BAF}
    \mathrm{BAF}_1=\frac{C_1}{\Cw^{o}}=B_1\,\frac{g_\mathrm{in}}{\,g_\mathrm{out}+\dot Q_\mathrm{TP}/M_1\,}.
\end{equation}
The root BAF is thus the binding capacity $B_1$ times an influx/(efflux $+$ xylem export) ratio.  The chain-length dependence observed empirically (concave/U-shaped for PFCA, monotone for PFSA) enters through the chain-length variation of $B_1$ and of $g_\mathrm{in}/g_\mathrm{out}$.
...
The present document fixes the model skeleton, the dimensionless structure, and the calibration design.  Items deferred to subsequent work include: the QSPR construction for $K_\mathrm{prot}$ and $K_\mathrm{PL}$ (Tier~3); an explicit soil-side sub-model for $\Cw^{o}(t)$ under paddy redox dynamics; the saturable (nonlinear) regime of the carrier and protein-binding terms; and numerical implementation with parameter estimation against rice field and lysimeter datasets.
