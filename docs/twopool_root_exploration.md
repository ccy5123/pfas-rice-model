...
- **What works:** the two-pool *structure* decouples root burden from shoot delivery, lets the
  model keep the monotone physical `f_xy_recommended`, and ties the best prior global fit (0.257
  excl PFDoDA). The root-matched test proves the structure is **sufficient** — matching the root
  exactly leaves the shoot essentially unchanged. Mass-conserving; the canonical core and
  `parameters.json` are untouched.
- **The one concrete open item (now well-posed):** the root-sink descriptor `k_seq(n, group)`
  must be **U-shaped**, not monotone — that is exactly why the linear global fit collapsed
  (`ks_b → 0`). The empirical per-congener `k_seq` (Result 2) are the anchors; the next step is a
  parsimonious U-form (e.g. `a·e^{−b(n−n₀)} + c·[high-chain rise] · head-group offset`, 4–5
  globals, the same family as the straw U-shape) re-fit with PFDoDA included. This is expected to
  recover the PFOS/PFUnDA separation that B/K_PL cannot — the genuinely PFAS-specific root term.
