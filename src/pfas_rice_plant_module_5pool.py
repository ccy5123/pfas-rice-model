...
class Compound:
    ...
    K_prot: float
    K_PL: float
    K_cw: float             # polysaccharide cell wall only (cellulose+hemicellulose+pectin)
    K_lignin: float         # lignin pool (hydrophobic aromatic sorbent; Mel 2024 direct Kd)
...
class Compartment:
    ...
    name: str
    theta: float            # fresh-weight water fraction [~0-1]
    f_prot: float           # protein mass fraction [kg/kg dw]
    f_PL: float             # phospholipid (membrane) mass fraction [kg/kg dw]
    f_cw: float             # polysaccharide cell-wall mass fraction [kg/kg dw]
    f_lignin: float         # lignin mass fraction [kg/kg dw]
...
def binding_factors(comps: list[Compartment], cmpd: Compound) -> np.ndarray:
    """B_k = theta_fw + (1-theta_fw)*(f_prot K_prot + f_PL K_PL + f_cw K_cw + f_lignin K_lignin)  [L/kg fw].

    FRESH-WEIGHT basis (NOTES path A): theta is the fresh-weight water fraction and
    f_* are DRY-weight mass fractions, rescaled by (1 - theta_fw) onto a per-kg-fresh
    basis -- consistent with the model's fresh-mass M. Compare to dw data via
    C_dw = C_fw / (1 - theta_fw).
    5-POOL: cell wall is split into polysaccharide (f_cw, K_cw) and lignin
    (f_lignin, K_lignin); lignin is a hydrophobic aromatic sorbent (Mel 2024 direct Kd).
    """
    return np.array([
        c.theta + (1.0 - c.theta) * (
            c.f_prot * cmpd.K_prot + c.f_PL * cmpd.K_PL
            + c.f_cw * cmpd.K_cw + c.f_lignin * cmpd.K_lignin)
        for c in comps
    ])
...
def _demo():
    ...
    cmpd = Compound(
        name="PFOA",
        K_prot=50.0, K_PL=100.0, K_cw=4.0, K_lignin=17.0,  # [L/kg] PROVISIONAL(PFOA): K_cw=poly-only, K_lignin=Mel; K_PL pending QC1
    ...
    # fraction; f_* = DRY-weight fractions; f_cw = polysaccharide, f_lignin separate.
    comps = [
        Compartment("root",  theta=0.90, f_prot=0.07, f_PL=0.015, f_cw=0.40, f_lignin=0.10),
        Compartment("stem",  theta=0.83, f_prot=0.05, f_PL=0.005, f_cw=0.58, f_lignin=0.14),
        Compartment("leaf",  theta=0.78, f_prot=0.10, f_PL=0.010, f_cw=0.48, f_lignin=0.08, S=20.0),
        Compartment("grain", theta=0.14, f_prot=0.09, f_PL=0.003, f_cw=0.025, f_lignin=0.010, S=2.0),
    ]
