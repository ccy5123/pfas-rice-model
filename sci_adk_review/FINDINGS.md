...
root 1.2 vs 69). PFDoDA 잔여는 **내부분배가 아니라 uptake(jR) 질량수지 한계** → 장쇄 흡수 강화/
비가역 흡착 등 *다른* 메커니즘 필요. **LC4 = CONTESTED**(부분 해법). in-sample. `sci-adk verify`
4 claim 재현(exit 0). 재현: `python sci_adk_review/build_longchain.py` (+ `validation/twopool_longchain.py`).
