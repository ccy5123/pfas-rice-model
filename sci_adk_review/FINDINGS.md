...
작동)이나, 현 **단일-풀 구현은 root를 희생**하고 **PFDoDA 잔여 floor**가 남는다(LC3 거부).
→ 후속: **2-풀(자유 + 지질결합) 분리**로 결합 풀이 root를 비우지 않고 shoot를 먹이게 하고,
PFDoDA 잔여 메커니즘(비가역/이력 흡착)을 별도로. in-sample(보정 데이터) 평가임. `sci-adk
verify` 3 claim 재현(exit 0). 재현: `python sci_adk_review/build_longchain.py`.

**관련 문헌 검색 (sci-adk literature 경로; `evi-lc-literature`)** — 발견은 agent
web_search, 기록은 sci-adk LITERATURE 증거(paperforge OA 취득은 비공개 `[tools]`라 미사용;
DOI만 기록 → 논문 draft 서지로 반영). 결과는 LC1·LC2를 **외부 corroborate**(novelty 주장
아님 — 메커니즘은 문헌 확립):
