...
for r in pfas-rice pfas-rice-longchain pfas-rice-carrier \
         ...
         pfas-rice-model-selection pfas-rice-risk-readiness pfas-rice-2pool-core \
...
**장쇄 "완전해법"의 동시-폐쇄 검정 (별도 run `pfas-rice-longchain-complete`, `build_longchain_complete.py`)**:
위 §7 결론은 "완전한 장쇄 해법 = 2-풀 + 지질로딩 + 능동운반체"를 **제안**했으나 세 레버(LC4·LC5b·LC6)는
각각 *고립*되어서만 검정됐다. 이 run은 셋을 **한 모델로 결합**해 정작 중요한 성질 — 장쇄(C10–C12)의
**root와 shoot(straw)를 동시에** 재현하는가 — 를 검정한다(`validation/longchain_complete.py`: 동족체마다
측정 root를 재현하는 운반체 배수를 찾고, 그 운반체에서 lipid `g_xy`(straw)/`g_ph`(grain)를 적합). 결과
(4 claim 전부 SUPPORTED, threshold 자동판정, `sci-adk verify` digest `4aafc495…`):
- **root 폐쇄**(long-chain log10 RMSE **0.002**) — 능동운반체가 장쇄 root를 들어올릴 수 있음(구성상).
- **grain 폐쇄**(RMSE **0.230**, ~factor 2) — lipid phloem 항으로.
- **그러나 shoot는 동시에 폐쇄되지 않음**(straw RMSE **0.386** > 0.3): root를 고정한 운반체가 mobile
  pool을 키워 **자유로딩 `f_xy·Cw_m`만으로도 관측 straw를 초과**하고, lipid `g_xy≥0`은 *뺄 수 없어*
  straw가 과대(**PFUnDA 3.3×, PFDoDA 2.3×**). 즉 운반체-root 결합이 **구조적으로 shoot를 과급**한다.
...
**★ 돌파구 — 장쇄는 사실 구조적으로 닫힌다 (별도 run `pfas-rice-risk-readiness`의 핵심 실험 `validation/longchain_closure.py`)**:
위 complete/decouple의 "동시-폐쇄 불가" 결론은 **f_xy를 (oryza값에) 고정한 채 뺄 수 없는 lipid(g_xy≥0)만
적합한 인공물**이었다. 장쇄에는 **두 개의 독립 물리량**이 있다 — (i) **낮은 f_xy**(strong root 잔류 = 낮은
TSCF)와 (ii) **강화된 능동운반체**(높은 측정 root를 만드는 흡수). 둘을 혼동(f_xy를 높게 강제)한 게 실패
원인이었다. **f_xy를 자유 적합**하면 표준 2-pool이 C10–C12의 root·straw·grain을 **동시에 재현**한다 —
장쇄 log10 RMSE **0.081**(PFDA 4.2/4.2·3.0/3.5·3.4/3.4; PFUnDA 19.6/19.5·7.3/8.2·3.1/3.1; PFDoDA
70.9/69.3·29.2/49.7·45.5/45.5, straw만 f_xy=1 천장 잔차). 단일-풀 core는 재현조차 못 했던(refit_oryza
천장에서 4–6× 미달) 것을 2-pool(mobile+bound)+carrier가 해낸다. **포화(3파라미터/3관측, DOF 0)** = H7
구조적 적합성을 C10–C12로 확장(재현이지 사전적 예측 아님). → 장쇄 "열린 문제"는 **구조적 적합성 수준에서
해소**됐다; 남은 건 per-congener 파라미터의 *예측가능성*(carrier QSPR 불가, f_xy 비단조).

**정식 배선 — 돌파구를 `src` 컴포넌트로 승격 (별도 run `pfas-rice-2pool-core`, digest `9f890a5d…`)**:
돌파구가 validation 스크립트에만 있으면 못 쓰므로 **`src/pfas_rice_two_pool.py`로 승격**(clean API +
`model_api.simulate_twopool_carrier`/`close_longchain_2pool` 훅; 카논 4pool_surf core는 **불변**=회귀 0).
sci-adk가 이 배선을 검증: 승격된 컴포넌트가 장쇄를 RMSE **0.081**로 재현(3/3 SUPPORTED), 폐쇄에는
**낮은 f_xy(전 장쇄 max 0.436<0.6, root 잔류) + 강화 운반체(PFDoDA 2.79×)** 두 독립 레버가 모두 필요함을
확인. 가드 `tests/test_two_pool.py`(3) + `test_2pool_core_run_reproduces`. 포화 적합 = 구조적 적합성(재현),
사전 예측 아님. → 장쇄 대응 모델이 **위해성 스크리닝용 재사용 컴포넌트**가 됐다.
...
## 10. 위해성 평가도구로서의 준비도 + 장쇄 돌파구 (`runs/pfas-rice-risk-readiness`)

"위해성 평가도구로 이용 가능해야 한다"는 요구를 *측정 데이터로* 판정. 식이 위해성의 핵심 구획은
**곡립(현미)**이다. 두 축으로 검정(4 claim 전부 SUPPORTED, `sci-adk verify` digest `bdcdacab…`):

1. **구조적 커버리지(돌파구)** — 2-pool + 자유 f_xy + 능동운반체가 전 C4–C12(장쇄 포함)를 재현
   (long-chain RMSE **0.081** < 0.2). 식이 관련 동족체 범위에 **구조적 사각지대 없음**(§7 돌파구). 단
   포화 적합 = *표현 가능성*(structural adequacy)이지 사전적 예측 아님.
2. **곡립 표본외 예측** — lipid 메커니즘(Kim 미적합)이 독립 **Kim 2019 현미 곡립 BAF**를 표본외로
   log10 RMSE **0.48(~factor 3)**, 신뢰부분집합(DF≥15%) **0.20(~factor 1.6)** 예측 → **스크리닝 적합**.
3. **정직한 보증수준(1급 가설)** — 최악 곡립 잔차 **PFOS endosperm ~5×**(log10 0.70 > 0.5) + GenX(ether)
   과대 → **스크리닝급이지 규제정밀급 아님**.

**판정**: 모델은 **스크리닝 수준의 식이 위해성 평가도구로 사용 가능**하다 — 곡립을 독립 데이터에서
~factor 3(신뢰부분 ~1.6)로 예측하고, 장쇄 구조 사각지대는 돌파구로 닫혔다. **단 동족체별 불확실성
계수**(통상 ~3×, PFSA endosperm·ether 최대 ~5×)를 적용해야 하며, 규제정밀 용도(정량 dose–response)는
추가 데이터(신뢰성 있는 2-pool root + ether/sulfonamide QSPR)가 필요하다. 즉 **"쓸 수 있다, 단 스크리닝급
+ 명시된 불확실성 계수"**가 정직한 답이다. 재현: `python sci_adk_review/build_risk_readiness.py`;
가드 `test_risk_readiness_run_reproduces`.
